-- 銘印鑽石 IMPRINT DIAMOND — Neon Postgres schema
-- Replaces Supabase (Postgres + Auth + PostgREST + RLS) with plain Postgres,
-- AND ports the full imprint-calculator (Flask) data model: dynamic product
-- catalog (admin-managed, real gold weights), cart, favorites, invite codes,
-- notifications, and the richer order/submission model. Authorization that
-- used to live in Supabase RLS / Flask-Login now lives in api/*.js request
-- handlers instead (see backend/lib/auth.js).
