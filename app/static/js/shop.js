...
function productImagesForColor(product, color) {
  if (product?.id && window.ShopAssets?.productImages) {
    const list = window.ShopAssets.productImages(product.id, product.defaultColor);
    if (list.length) {
      const c = color || product.defaultColor || 'white';
      const primary = window.ShopAssets.productImage(product.id, c, product.defaultColor);
...
function productImageUrl(product, color) {
  if (product?.id && window.ShopAssets?.productImage) {
  ...
  if (product?.id && window.ShopAssets) return window.ShopAssets.styleThumb(product.id);
  return '';
}
...
function renderTypeCards() {
  ...
  products.forEach(product => {
    ...
    img.loading = "lazy";
    window.ShopAssets?.attachImageFallback(img, window.ShopAssets.styleThumb(styleId));
